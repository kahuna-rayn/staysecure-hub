-- Recreate missing RLS policies (drop and recreate to avoid conflicts)

-- Breach management policies (these were likely dropped)
...
ON public.languages FOR INSERT, UPDATE, DELETE
USING (has_role(auth.uid(), 'super_admin'::app_role));
...
ON public.key_dates FOR INSERT, UPDATE, DELETE
USING (has_role(auth.uid(), 'super_admin'::app_role) OR has_role(auth.uid(), 'client_admin'::app_role));
